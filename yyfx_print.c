...
Type StructSpecifier(struct gramtree* node)
{
	...
	//printf("before search_repeat name is %s\n",struct_name);
	if(search_repeat(struct_name)){
    printf("Error type 16 at Line %d: Duplicated name \"%s\".\n",cur->lineno,struct_name);
    delete_struct_space(struct_name);
    return NULL;
}
...
void FunDec(Type return_type,struct gramtree* tree,int is_defining){
    ...
    if(is_defining==0){
	    ...
	    //printf("lalala %s\n",func_name);
            insert_func_unit_bytype(func_name,return_type,0,NULL,0,tree->lineno);
            return;
        }
        //有参数
        char *func_name=tree->leftchild->content;
        ...
        *var_num=0;
        ...
        //定义对应的函数
        insert_func_unit_bytype(func_name,return_type,*var_num,Typelist,0,tree->lineno);
        return;
    }
    if(tree->leftchild->rightchild->rightchild->rightchild==NULL){
        //无参数
        char *func_name=tree->leftchild->content;
//printf("fun_name: %s\n",func_name);
        insert_func_unit_bytype(func_name,return_type,0,NULL,1,tree->lineno);
...
//	if(Typelist[pi]==NULL){//printf("Typelist[%d] is NULL\n",pi);}
    ...
    insert_func_unit_bytype(func_name,return_type,*var_num,Typelist,1,tree->lineno);
    extern struct node* func_top;
    //printf("func_top name:%s\n",func_top->vi_name);
    insert_space_unit(1,func_name);//加入一个域 ok
    //在此处直接访问space链表需要外部引用变量吗
    int i=0;
	//原来错误写法是yatop->forw  那样栈顶是函数！不要移动函数
	struct space_unit * yatop=top->forw_space;
    struct node *topnode=yatop->forw->space_forw_node;
    struct node *findnode=yatop->forw->space_forw_node;
    struct node *beforefindnode=yatop->forw->space_forw_node;    
//移动*var_num个，得到第一个不用删除的变量结点
    for(i=0;i<*var_num;i++){
	//printf("current node is %s\n",findnode->vi_name);
        findnode->space_deep=space_deep;//加入新的域（深度+1）
	//printf("新深度为%d\n",space_deep);
        //space_deep也是全局变量
        findnode=findnode->space_forw_node;
	if (findnode==NULL) {
    ...
    for(i=0;i<(*var_num)-1;i++){
        beforefindnode=beforefindnode->space_forw_node;
    }
    //findnode：第一个不用删除的变量结点
    //beforefindnode 他之前的那个
	
yatop->forw->space_forw_node=findnode;//这里也是！不能没有->space_forw_node
if (yatop->forw==NULL) {
        //printf("yatop->forw==NULL\n");
    }
    else{
        //printf("yatop->forw->vi_name=%s\n",yatop->forw->vi_name);
    } 
   beforefindnode->space_forw_node=NULL;
    top->forw=topnode;//函数域刚加入，目前肯定没有任何东西
}
Type* VarList(struct gramtree* tree, int *var_num){
    //printf("VarList\n");
    //最开始传的是0
...
Type ParamDec(struct gramtree* tree){//ParamDec->Specifier VarDec
    //printf("ParamDec\n");
    ...
    char *shadiao = NULL;
    //获取变量名
    char * var_name=(char*)malloc(sizeof(char)*40);
    struct gramtree* tmp_tree=tree->leftchild->rightchild;//VarDec
    while (tmp_tree!=NULL) {
        if(strcmp(tmp_tree->name,"ID")==0){
            //ID
            var_name=tmp_tree->content;
	   break;        
}
	tmp_tree=tmp_tree->leftchild;
    ...
    if(dem==0){
        //插入非数组变量
        insert_variable_unit_bytype(var_name,specifier);
        return specifier;
    }
    else{
        ...
        //插入数组变量
        insert_array_unit(var_name,dem,new_array_type);
...
void Dec(struct gramtree* node,Type type_,int flag)  //flag 1：结构体  0：函数
{
    ...
    dimension = VarDec(cur,&name_,dimension);
	if(search_repeat(name_)){
    if(flag) printf("Error type 15 at Line %d: Redefined field \"%s\".\n",cur->lineno,name_);
...
if(Count!=search_func(cur->content)->param_size){printf("Error type 9 at Line %d: Function is not applicable for arguments.\n",cur->rightchild->rightchild->lineno);return NULL;}                       
 for(i=0;i<Count;i++){
...
//			    printf("i:%d\n",i);
		    ...
		    *flag = (*flag)&1;
			    ...
			    tflag = 1;
                            if(judge_type(Exp(cur->rightchild->rightchild,&tflag))!=0){
                                printf("Error type 12 at Line %d:%s is not an integer.\n ",cur->lineno,cur->rightchild->rightchild->leftchild->content);
                                return NULL;
                            }
                            else return arrayMem(t,1);
                        }
                        else{
                            printf("Error type 10 at Line %d:%s is not an array.\n ",cur->lineno,cur->leftchild->content);
                            return NULL;
                        }
                    }
		    else return NULL;
	...
	//		printf("Exp DOT ID Exp type is 3\n");
                        if(structMem(t,1,cur->rightchild->rightchild->content)==NULL){///error 14访问结构体中未定义过的域
                            printf("Error type 14 at Line %d:Non-existent field \"%s\"\n",cur->lineno,cur->rightchild->rightchild->content);
                            return NULL;
                        }
                        else
			{
