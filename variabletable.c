...
//	printf("insert_func_bybyte\n");
    ...
    int flag=able_define_func_bytype(func_name, return_type, param_size, param_types, is_defining,lineno);
//printf("1\n");
    if(!flag){
        //printf("函数定义/声明失败\n");
        return;
...
void check_all_func_defined(){
    ...
    while(tmp_func!=NULL){
        if(!tmp_func->is_defined){
            //printf("存在未定义函数%s\n",tmp_func->vi_name);
            count_no_defined++;
		printf("Error type 18 at Line %d: Undefined function %s\n",tmp_func->func_lineno,tmp_func->vi_name);
        }
        tmp_func=tmp_func->nxt_func_node;
    }
...
//if(func_top!=NULL)
	...
	printf("Error type 19 at Line %d: Inconsistent declaration of function %s\n",lineno,rep_func->vi_name);
		...
		printf("Error type 19 at Line %d: Inconsistent declaration of function %s\n",lineno,rep_func->vi_name);//printf("Error type 19 at Line %d: Inconsistent declaration of function %s\n",lineno,rep_func->vi_name);
                    return 0;
                }
            }
            //printf("与之前的同名函数声明一致\n");
...
int search_repeat(char* vi_name){
	...
	//printf("当前深度为%d 同名变量深度为%d\n",space_deep,pet->space_deep);
            //深度一致，同域同名！
            //printf("error3 有（深度相同）同名变量%s，不能重复定义\n",pet->vi_name);
            return 1;
        }
        //不同域的同名，可以忽略
        //printf("相同作用域内无同名变量\n");
...
void delete_space_unit(int is_struct,...){
    ...
    if(is_struct){//如果这个域对应一个结构体
      ...
      //      printf("有重名结构体或变量\n");
        ...
        struct node *p=top->forw;
        while (p!=NULL) {
	//printf("in delete space p is not null");
          //  printf("name is %s\n",p->vi_name);
            top->forw=p->space_forw_node;//更换表头为下一个node
            //再在hash中删除p
            int val=pjw_hash(p->vi_name);
